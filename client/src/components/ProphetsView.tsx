...
export function ProphetsView() {
  ...
  const badgeTiers = [
    { label: '3', threshold: 3, color: 'text-[#F4D03F]' },
    { label: '6', threshold: 6, color: 'text-[#F4D03F]' },
    { label: '12', threshold: 12, color: 'text-[#F4D03F]' },
  ];

  return (
    <div className="mx-auto max-w-3xl px-4 pb-8 pt-6 animate-fade-in">
      <div className="mb-4 text-center">
        <h2 className="font-display text-2xl font-bold text-[#D4AF37]">{t('prophets.title')}</h2>
      ...
      {(() => {
        ...
        return (
          <div className="card mb-4 overflow-hidden p-4" style={{ borderColor: 'rgba(212,175,55,0.7)', background: '#112925' }}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="truncate text-sm font-bold text-white">
                  <span className="font-quran text-lg text-[#D4AF37]" style={{ textShadow: '0 0 8px rgba(212,175,55,0.3)' }} dir="rtl">{featured.prophet.nameAr}</span>{' '}
                  {featured.prophet.nameFr}
                </p>
                <p className="text-[10px] text-[#A3B1AC]">
                  {fdone ? '✅ ' + t('prophets.weeklyDone') : t('prophets.weeklyDaysLeft') + ' : ' + featured.daysLeft}
                </p>
              </div>
              <button onClick={() => setSelected(featured.prophet)}
                className={'btn-gold shrink-0 rounded-xl px-3 py-2 text-[11px] transition hover:shadow-[0_0_20px_rgba(212,175,55,0.35)] ' + (fdone ? '!bg-[#1F6E5C]' : '')}>
      ...
      <div className="card mb-4 p-4">
        <div className="flex items-center justify-between">
          <p className="font-display text-xs font-bold text-[#D4AF37]">📜 {t('prophets.progressTitle')}</p>
          <p className="text-xs font-semibold text-stone-300">{completedCount}/{totalStories}</p>
        </div>
        <div className="mt-2 h-2.5 w-full overflow-hidden rounded-full" style={{ background: '#2A4A43' }}>
          <div className="h-full rounded-full transition-all duration-500" style={{ width: globalPct + '%', background: 'linear-gradient(90deg, #D4AF37, #F4D03F)' }} />
        </div>
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          <span className="text-[10px] text-stone-500">{t('prophets.badgeTitle')}</span>
          {badgeTiers.map((b) => (
            <span key={b.threshold} className={
              'flex h-8 w-8 items-center justify-center rounded-full text-[10px] font-bold ' + (completedCount >= b.threshold
                ? 'border border-[#D4AF37] bg-[#112925] text-[#F4D03F]'
                : 'border border-[#2A4A43] bg-[#112925] text-stone-500 opacity-60')
            }>
              {b.threshold}
            </span>
          ))}
        ...
        {PROPHETS.map((p) => {
          ...
          return (
            <button key={p.name} onClick={() => setSelected(p)}
              ...
              <div className="flex items-center gap-3">
                <div className="font-quran text-2xl shrink-0 text-[#D4AF37]" style={{ textShadow: '0 0 8px rgba(212,175,55,0.3)' }} dir="rtl">{p.nameAr}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <div className="font-display font-bold text-white">{p.nameFr}</div>
                    {done && <span className="text-xs">✓</span>}
                  </div>
                  <div className="truncate text-[11px] text-[#A3B1AC]">{p.title}</div>
                  <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full" style={{ background: '#2A4A43' }}>
                    <div className="h-full rounded-full transition-all duration-500" style={{ width: pct + '%', background: '#1F6E5C' }} />
                  </div>
                </div>
                <span className="text-xs" style={{ color: done ? '#D4AF37' : '#A3B1AC' }}>{done ? '✓' : '→'}</span>
              </div>
            </button>
          );
        })}
