...
export function DevotionProvider({ children }: { children: ReactNode }) {
  ...
  const latestRef = useRef({ prayers, quests, achievements });
  const scopeRef = useRef(scope);

  useEffect(() => { latestRef.current = { prayers, quests, achievements }; }, [prayers, quests, achievements]);
  ...
  const value = useMemo<DevotionStore>(() => ({
    ...
    togglePrayer,
    toggleQuest,
  }), [prayers, quests, achievements, busy, syncState, lastSyncAt, pendingCount, refresh, togglePrayer, toggleQuest]);
