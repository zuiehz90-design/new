...
export function HijriCalendarView() {
  ...
  // Explication dépliée (clé « mois-jour » de l'événement)
  const [expanded, setExpanded] = useState<string | null>(null);
  // Quiz de connaissances en cours
  const [quiz, setQuiz] = useState<{ name: string; questions: EventQuizQuestion[] } | null>(null);

  /** Bloc explicatif dépliable d'un événement (histoire, signification, pratiques). */
  const renderExplanation = (e: IslamicEvent) => {
    const key = `${e.month}-${e.day}`;
    const details = getEventExplanation(e.month, e.day);
    if (!details) return null;
    const isOpen = expanded === key;
    const questions = getEventQuiz(e.month, e.day);
    return (
      <div className="mt-2">
        <div className="flex items-center gap-3">
          <button
            onClick={() => setExpanded(isOpen ? null : key)}
            className="text-[11px] font-semibold text-gold-400 underline-offset-2 hover:underline"
          >
            {isOpen ? `▴ ${t('hijri.detailsHide')}` : `▾ 📖 ${t('hijri.details')}`}
          </button>
          {questions && (
            <button
              onClick={() => setQuiz({ name: lang === 'ar' ? e.nameAr : e.name, questions })}
        ...
        {isOpen && (
          <div className="mt-2 space-y-2 animate-fade-in">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wide text-gold-400">{t('hijri.sectionHistory')}</p>
              <p className="mt-0.5 text-[11px] leading-relaxed text-stone-300">{details.history}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wide text-gold-400">{t('hijri.sectionMeaning')}</p>
              <p className="mt-0.5 text-[11px] leading-relaxed text-stone-300">{details.meaning}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-400">{t('hijri.sectionPractices')}</p>
              <ul className="mt-1 space-y-0.5">
                {details.practices.map((p, i) => (
                  <li key={i} className="text-[11px] leading-relaxed text-stone-300">
