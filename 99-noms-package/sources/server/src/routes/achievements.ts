...
export function computeStats(userId: number): Stats {
  ...
  const row = db.prepare(`
    SELECT
      ...
      (SELECT COUNT(*) FROM quests WHERE user_id = ? AND done = 1 AND type = 'quran') AS quran_quests,
      (SELECT COUNT(*) FROM quests WHERE user_id = ? AND done = 1 AND type = 'dhikr') AS dhikr_quests,
      (SELECT COUNT(*) FROM quiz_completions WHERE user_id = ? AND score > 0) AS stories_done,
      (SELECT COUNT(*) FROM quiz_completions WHERE user_id = ? AND score = total AND total > 0) AS quiz_perfect
  `).get(userId, userId, userId, userId, userId, userId) as {
    total_prayers: number; quests_done: number; quran_quests: number; dhikr_quests: number;
    stories_done: number; quiz_perfect: number;
  } | undefined;
  ...
  return {
    ...
    quranQuests: row?.quran_quests ?? 0,
    dhikrQuests: row?.dhikr_quests ?? 0,
    quizPerfect: row?.quiz_perfect ?? 0,
  };
}
...
export function checkAchievements(userId: number, statsOverride?: Stats): string[] {
  ...
  const inputs: BadgeInputs = {
    ...
    quranQuests: stats.quranQuests,
    dhikrQuests: stats.dhikrQuests,
    quizPerfect: stats.quizPerfect,
  };
